...
class CrapsView(nextcord.ui.View):
    ...
    async def on_timeout(self):
        ...
        await self.interaction.followup.send("Craps game timed out.")
    ...
    async def red(self, button: nextcord.ui.Button,
        ...
        if coins >= self.amount:
            ...
            if result == "red":
                ...
                await message.edit(
                    content=f"The roulette landed on 🔴 Red! You won **{self.amount}** :coin: coins!"
                )
            else:
                new_coins = coins - self.amount
                data["coins"] = new_coins
                write_user_data(user_id, data)
                await message.edit(
                    content=f"The roulette landed on ⚫ Black! You lost **{self.amount}** :coin: coins."
                )
        else:
            await interaction.response.send_message(
    ...
    async def black(self, button: nextcord.ui.Button,
        ...
        if coins >= self.amount:
            ...
            if result == "black":
                ...
                write_user_data(user_id, data)
                await message.edit(
                    content=f"The roulette landed on ⚫ Black! You won **{self.amount}** :coin: coins!"
                )
            else:
                new_coins = coins - self.amount
                data["coins"] = new_coins
                write_user_data(user_id, data)
                await message.edit(
                    content=f"The roulette landed on 🔴 Red! You lost **{self.amount}** :coin: coins."
                )
        else:
            await interaction.response.send_message(
                f"You do not have enough :coin: coins. Check your balance. Place a lower bet or claim the daily reward!"
    ...
    async def green(self, button: nextcord.ui.Button,
        ...
        if coins >= self.amount:
            ...
            if result == "green":
                ...
                write_user_data(user_id, data)
                await message.edit(
                    content=f"The roulette landed on 🟢 Green! You won **{self.amount * 2}** :coin: coins!"
                )
            else:
                new_coins = coins - self.amount
                data["coins"] = new_coins
...
@bot.slash_command(
    name="coinflip",
    description="Choose 'heads' or 'tails', wager an amount, and flip a coin to win big")
...
def generate_result():
    if random.random() < .26:
        ...
        return [random.choice(display_emojis) for _ in range(3)] + [0]
...
async def slots(interaction: nextcord.Interaction, amount: int):
    ...
    if len(result) == 4 and result[0] == result[1] == result[2]:
        winnings = amount * result[3]
        user_data['coins'] += amount + winnings
        write_user_data(user_id, user_data)
        await interaction.response.send_message(
            f"The slot machine spins...\n\n{result[0]} {result[1]} {result[2]}\n**{result[3]}x  Multiplier**\n\nYou won **{winnings}** :coin: coins!"
        )
    else:
        await interaction.response.send_message(
...
# Create a default empty user_data.json file if it doesn't exist
...
def main():
    ...
    if not token:
        ...
        print("You can create a Discord bot and get a token at https://discord.com/developers/applications")
        return
    host()
    bot.run(token)
